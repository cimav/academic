...
@seminars = Advance.select("distinct advances.*").joins(:student=>:program).joins(:student=>{:term_students=>:term}).where("programs.level in (:level) AND (tutor1=:id OR tutor2=:id OR tutor3=:id OR tutor4=:id OR tutor5=:id) AND advances.advance_type=3 AND advances.status in (:status) AND terms.status in (:t_status) AND students.status in (:student_status)",:id=>@staff.id,:level=>[1,2],:status=>['P','C'],:t_status=>[1,2,3],:student_status=>[1,6]).where(:advance_type=>3)
