class StaffsController < ApplicationController
  ...
  def grades
    ...
    @protocols = Advance.select("distinct advances.*").joins(:student=>:program).joins(:student=>{:term_students=>:term}).where("(curdate() between terms.advance_start_date and terms.advance_end_date) AND terms.status in (1,2,3) AND programs.level in (:level) AND (tutor1=:id OR tutor2=:id OR tutor3=:id OR tutor4=:id OR tutor5=:id) and advances.status in (:status)",:id=>@staff.id,:level=>[1,2],:status=>['P','C']).where(:advance_type=>2)
    
    @seminars = Advance.select("distinct advances.*").joins(:student=>:program).joins(:student=>{:term_students=>:term}).where("(advances.advance_date between terms.start_date and terms.end_date) AND programs.level in (:level) AND (tutor1=:id OR tutor2=:id OR tutor3=:id OR tutor4=:id OR tutor5=:id) AND advances.advance_type=3 AND advances.status in (:status) AND terms.status in (:t_status)",:id=>@staff.id,:level=>[1,2],:status=>['P','C'],:t_status=>[1,2,3]).where(:advance_type=>3)
